...
d_1p = math.sqrt(a_1**2 + (h_1-b)**2)
d_2p = math.sqrt(a_3**2 + (h_3-b)**2)
d_3p = math.sqrt(a_2**2 + (h_2-b)**2)
A_1p = h_1 / d_1p**2
A_2p = h_3 / d_2p**2
A_3p = h_2 / d_3p**2
V_op_real = 0.25 * 110 * b * (A_1p - 0.5*(A_2p+A_3p))
V_op_imag = 0.25 * 110 * b * (math.sqrt(3)/2*(A_2p-A_3p))
V_op = complex(V_op_real, V_op_imag)
amplituda = abs(V_op)
uhol_v_rad = cmath.phase(V_op)
uhol_v_stupnoch = math.degrees(uhol_v_rad)
print('Napätie pre nezatazene potrubie  CIGRE  variant je: {} ∠ {} '.format(amplituda, uhol_v_stupnoch))
